...
During lesson 2 the main arguments you will face are the following:
...
Solidity has two locations to store variables: storage and memory. Storage variables are permanently stored on the blockchain, while memory variables are temporary and erased between external function calls.
...
Functions can return multiple values in Solidity. Use multiple assignment to retrieve and store multiple return values, allowing efficient work with functions providing multiple results.
